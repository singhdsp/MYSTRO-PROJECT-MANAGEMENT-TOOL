...
import { createIn } from "../server/attendance";
...
export default function StartDay({ isOpen, setIsOpen, refreshing, setRefreshing }) {
  ...
  const t = useTranslations("StartDay");
  ...
  const handleSubmit = async (event) => {
    ...
    try {      
      ...
      if (result.status === "success") {       
        ...
        notyf.error(result.error || "Failed to upload image. Please try again.");
      }
    } catch (err) {
      notyf.error("An error occurred while uploading the image. Please try again.");
    } finally {
    ...
    >
      <Card className="max-w-sm mx-auto">
        <CardHeader className="flex flex-row items-center justify-between">
          ...
          <CardTitle className="text-xl font-bold">{t('StartDay')}</CardTitle>
